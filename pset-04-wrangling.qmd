...
```{r}
source("census-key.R")
#This line of code tells R to run the census key and loads it.

census_key <- "A_CENSUS_KEY_THAT_WORKS"
#I'm bring this line of code here so that it can run in later chunks
```
...
url <-paste0(url,                         #endpoint
  "?get=POP_2020,POP_2021,NAME",          #requested variables, vintage population estimate 2021-2022
  "&for=state:*",                         #requested range  
  "&key=", census_key                     #the local key
)
#doing so create the full url that runs on any laptop with the specified key 

request <-request(url)                    #create a request
```
...
```{r}
response <- request |> req_perform()      #perform the actual request
...
```{r}
cases|>
  filter(year(date) %in% c(2020, 2021)) |>                    #filter the year to only 2020-2021
  mutate(year = year(date)) |>                                #add a year column in cases for join
  left_join(population,by = c("state","year")) |>             
  mutate(cases_rate= (cases/population)*100000) |>            #calculate the rate as requested
  filter(!is.na(region_name))|>
  ggplot(aes(x=date, y=cases_rate,col=state))+ 
  geom_line()+
  facet_wrap(~region_name)+                                   #stratify by region_name
  labs(
...
library(knitr)
cases |> 
  filter(year(date) %in% c(2020, 2021)) |>                #filter to only 2020 and 21
  mutate(year = year(date),
         month = month(date)) |>                          #create month and year
  group_by(year,month)|>
  summarize(total=sum(cases,na.rm=TRUE),                  #calculate the sum for each month in each yr
            .groups="drop")|>
...
deaths_raw <- request(deaths_url) |>
  req_url_query(`$limit` = 10000000000) |>     #like previous question, set the limit high to grab data
  req_perform() |>
  resp_body_json(simplifyVector = TRUE) 

death <- deaths_raw|>
  select(state,end_date,covid_19_deaths)|>             #select data and rename them
...
death|>
  group_by(state)|>
  summarize(total=sum(deaths,na.rm=TRUE),            #calculate the total death by state
            .groups="drop")|>
  arrange(desc(total))|>
  slice_head(n = 10) |>                             #arrange by descending+grab the top 10
